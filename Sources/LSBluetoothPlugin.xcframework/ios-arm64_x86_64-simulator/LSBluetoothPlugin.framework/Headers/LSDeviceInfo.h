...
#import "IBGattDevices.h"
#import "LSUserInfo.h"

@interface LSDeviceInfo : IBGattDevices
...
@property(nonatomic, assign) BOOL syncSleepTime;                     //是否同步睡眠时间
@property(nonatomic, assign) LSHeadsetType headsetType;              //耳机类型
@property(nonatomic, assign) NSUInteger state;                       //绑定状态，1表示已绑定，0表示未绑定
 ...
 * 血糖仪设备型号判断
 */
-(BOOL)isBloodGlucoseMeter;

/**
 * 判断是否是A6+协议支持多用户
 */
-(BOOL)isSupportMultiUsers;

/**
 * 根据广播名判断当前设备是否为耳机或助听器
 */
-(BOOL)isHeadsetDevice;

/**
 * 判断广播名是否为未烧码设备，MAC=FFFFFFFFFFFF
 */
-(BOOL)isUnburnedDevice;
@end
