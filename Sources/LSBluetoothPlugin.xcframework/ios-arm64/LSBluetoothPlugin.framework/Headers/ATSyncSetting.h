 ...
 * 重复方式转星期
 */
-(NSArray *_Nonnull)toWeekDayWithRepeatType:(NSUInteger)value;
 ...
 * 数据包序列号
 */
@property(nonatomic,assign) int packetSeq;
...
#pragma mark - ATDataSycResetSetting

/**
 * 乐心手环6S数据同步重置指令
 * 设置后可以重复拉取设备端已同步过的历史数据
 */
@interface ATDataSyncResetSetting : ATSyncSetting

@property(nonatomic,assign) BOOL enable;

@end

#pragma mark - ATSamplingSetting

/**
 * PPG数据采样设置
 */
@interface ATSamplingSetting : ATSyncSetting

@property(nonatomic,assign) BOOL enable;            //true表示开始采集，false表示停止采集

@property(nonatomic,assign) ATDataSampling type;    //采集类型
@end
